...
int init_sem_names(t_monitor *monitor)
{
    ...
    if (!monitor->sem_names[0] || !monitor->sem_names[1] || !monitor->sem_names[2] || (monitor->eat_complete != NONE && !monitor->sem_names[3]))
    {
        free_names(monitor);
        return (0);
    }
...
int	init_semaphores(t_monitor *monitor)
{
	if (!init_sem_names(monitor))
        return (0);
	monitor->forks = sem_open(monitor->sem_names[0], O_CREAT, 0644, monitor->p_num);
	if (monitor->forks == SEM_FAILED)
		return (0);
	monitor->print_sem = sem_open(monitor->sem_names[1], O_CREAT, 0644, 1);
	if (monitor->print_sem == SEM_FAILED)
		return (cleanup_semaphores(monitor, (int []){1, 0, 0, 0}), 0);
	monitor->start_sem = sem_open(monitor->sem_names[2], O_CREAT, 0644, 0);
	if (monitor->start_sem == SEM_FAILED)
		return (cleanup_semaphores(monitor, (int []){1, 1, 0, 0}), 0);
	if (monitor->eat_complete != NONE)
	{
		monitor->eat_sems = sem_open(monitor->sem_names[3], O_CREAT, 0644, 0);
